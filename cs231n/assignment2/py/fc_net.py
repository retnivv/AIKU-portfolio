...
class FullyConnectedNet(object):
    ...
    def loss(self, X, y=None):
        ...
        # {affine - [batch/layer norm] - relu - [dropout]} x (L - 1) - affine - softmax
        # 💡INSIGHT : 
        # 지금 수강 중인 자료구조 강의에서 배운 stack 개념이 떠올라서 활용해봤다.
        ...
        for i in range(self.num_layers - 1):
            # affine
            layer_out, cache_temp = \
            affine_forward(layer_out, self.params[f"W{i+1}"], self.params[f"b{i+1}"])
            cache.append(cache_temp)
            # batch/layer norm
            if (self.normalization == "batchnorm"):    
                layer_out, cache_temp = batchnorm_forward(
                    layer_out, 
        ...
        # last_layer
        grad, grads[f"W{self.num_layers}"], grads[f"b{self.num_layers}"] = \
        ...
        for i in reversed(range(self.num_layers - 1)):
            ...
            # batch/layer norm
            if (self.normalization == "batchnorm"):
                grad, grads[f"gamma{i+1}"], grads[f"beta{i+1}"] = \
                batchnorm_backward_alt(grad, cache.pop())
            # affine
            grad, grads[f"W{i+1}"], grads[f"b{i+1}"] = \
            affine_backward(grad, cache.pop())
